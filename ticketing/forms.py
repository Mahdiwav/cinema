...
class ShowTimeSearchForm(forms.Form):
    ...
    sale_is_open = forms.BooleanField(label='فقظ سانس های قابل خرید', required=False)
    movie_length_min = forms.IntegerField(label='حداقل زمان فیلم', min_value=0, max_value=500, required=False)
    movie_length_max = forms.IntegerField(label='حداکثر زمان فیلم', min_value=0, max_value=500, required=False)
